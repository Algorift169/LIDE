 ...
 * upload.h
 * 
 * Upload tracking interface definitions
 *
...
/**
 * Initializes the upload speed monitor.
 * Detects the active network interface and captures baseline byte counts.
 * Must be called before any other upload functions.
 *
...
/**
 * Returns current upload speed as a formatted string with appropriate units.
 *
 ...
 *
 * @sideeffect Updates internal statistics state via get_upload_speed().
 */
const char* get_upload_speed_formatted(void);

/**
 * GLib timeout callback that refreshes upload statistics without calculating speed.
